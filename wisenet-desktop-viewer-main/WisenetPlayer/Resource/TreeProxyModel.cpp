...
bool TreeProxyModel::lessThan(const QModelIndex &left, const QModelIndex &right) const
{
    ...
    if(leftTreeItem->itemType() == (int)ItemType::ResourceRoot || leftTreeItem->itemType() == (int)ItemType::LocalFileRoot)
        return false;

    if(rightTreeItem->itemType() == (int)ItemType::ResourceRoot || rightTreeItem->itemType() == (int)ItemType::LocalFileRoot)
        return false;
...
bool TreeProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
    ...
    if(m_tempText != "") {
        ...
        if(itemInfo->itemType() == (int)ItemType::ResourceRoot || itemInfo->itemType() == (int)ItemType::LocalFileRoot)
            return false;
    }
