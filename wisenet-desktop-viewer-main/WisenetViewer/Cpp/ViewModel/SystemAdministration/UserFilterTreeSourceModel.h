...
class UserFilterTreeSourceModel : public BaseTreeModel
{
    Q_OBJECT
public:
    explicit UserFilterTreeSourceModel(QObject *parent = nullptr);
    virtual ~UserFilterTreeSourceModel();

    Qt::ItemFlags flags(const QModelIndex &index) const Q_DECL_OVERRIDE;

    Q_INVOKABLE QModelIndex getIndexByValue(QString event);

    Q_INVOKABLE void setUserData(QSet<QString> users);
