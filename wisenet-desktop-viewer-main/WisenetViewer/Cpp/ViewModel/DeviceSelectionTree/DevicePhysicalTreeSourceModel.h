...
class DevicePhysicalTreeSourceModel : public BaseTreeModel
{
    Q_OBJECT

public:
    explicit DevicePhysicalTreeSourceModel(QObject *parent = nullptr);
    virtual ~DevicePhysicalTreeSourceModel();


    Qt::ItemFlags flags(const QModelIndex &index) const Q_DECL_OVERRIDE;

    Q_INVOKABLE QModelIndex getModelIndex(QString guid);
    Q_INVOKABLE void readDevices(bool all = true);
