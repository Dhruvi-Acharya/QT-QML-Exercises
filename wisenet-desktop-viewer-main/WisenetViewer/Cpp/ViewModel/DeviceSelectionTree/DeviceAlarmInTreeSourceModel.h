...
class DeviceAlarmInTreeSourceModel : public BaseTreeModel
{
    Q_OBJECT

public:
    explicit DeviceAlarmInTreeSourceModel(QObject *parent = nullptr);
    virtual ~DeviceAlarmInTreeSourceModel();


    Qt::ItemFlags flags(const QModelIndex &index) const Q_DECL_OVERRIDE;

    Q_INVOKABLE QModelIndex getModelIndex(QString guid);
    Q_INVOKABLE void readAlarmIn(bool isNetworkAlarmIn = false);
...
private:
    ...
    void addTreeDeviceAlarmInItem(const Wisenet::Device::Device& device,
                           const Wisenet::Device::DeviceStatus& deviceStatus);
